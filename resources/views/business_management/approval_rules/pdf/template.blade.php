...
<body>
    ...
    @else
        <table class="data">
            ...
            <tbody>
                @foreach ($approval_rules as $item)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    ...
                                    @case('work_type')      {{ $item->workType?->code ?? __('approval_rules.all_work_types') }} @break
                                    @case('approver_role')  {{ $item->approver_role }} @break
                                    @case('priority_level') {{ $item->priority_level }} @break
                                    @case('is_required')    {{ $item->is_required ? __('approval_rules.required') : __('approval_rules.optional') }} @break
                                    @case('is_active')
                                        <span class="{{ $item->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $item->is_active ? __('global.active') : __('global.inactive') }}
                                        </span>
                                    @break
