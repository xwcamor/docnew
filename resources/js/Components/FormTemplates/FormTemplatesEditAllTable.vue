<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de FormTemplates. Recibe `draft` por
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('form_templates.table_headers.editable_name') }}</th>
                <th class="col-cod">{{ $t('form_templates.code') }}</th>
                <th class="col-status">{{ $t('form_templates.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('form_templates.edit_all_no_results') }}
    </div>
</template>
