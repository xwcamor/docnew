...
const onMenu = ({ key }) => {
    if (key === 'view')       router.visit(route('business_management.companies.show', props.record.slug));
    else if (key === 'edit')  router.visit(route('business_management.companies.edit', props.record.slug));
...
<template>
    ...
    <div v-else-if="isMobile" class="row-actions-mobile" @click.stop>
        <Tooltip :title="t('global.view')">
            <Link :href="route('business_management.companies.show', record.slug)">
                <Button
                    type="text"
                    class="row-icon-btn"
    ...
    <Space v-else :size="4" class="row-actions-desktop" @click.stop>
        ...
        <Tooltip v-if="canEdit && canManage && !isLocked" :title="t('global.edit')">
            <Link :href="route('business_management.companies.edit', record.slug)">
