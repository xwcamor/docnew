<script setup>
/**
 * Borrado definitivo, con tres cierres: hay que teclear el código del rol que
 * firma (es lo que identifica la regla a ojo en la papelera), el motivo es
 * obligatorio y de al menos 10 caracteres, y solo el super llega hasta aquí.
 */
...
const okDisabled = (target) => !target
    || form.value.name_confirmation !== target.approver_role
...
<template>
    ...
    >
        ...
        <p class="force-msg">
            {{ $t('global.force_delete_name_prompt', { name: target?.approver_role }) }}
        </p>
        <Input
            v-model:value="form.name_confirmation"
            :placeholder="target?.approver_role"
