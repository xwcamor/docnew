<script setup>
/**
 * Tabla editable en línea. Recibe el borrador por v-model y los predicados
 * isDirty/isDuplicate del composable. El código va como texto: es una clave, y
 * se cambia desde la ficha del rol, de una en una y a conciencia.
 */
...
<template>
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-cod">{{ $t('approver_roles.code') }}</th>
                <th class="col-name">{{ $t('approver_roles.table_headers.editable_name') }}</th>
                <th class="col-status">{{ $t('approver_roles.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
    ...
    <div v-else class="empty">
        {{ $t('approver_roles.edit_all_no_results') }}
    </div>
</template>
...
/* Tablet: se esconde el ID (dato técnico) y se deja el código, que es lo que
   identifica la fila para quien la está editando. */
