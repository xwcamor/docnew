<script setup>
/**
 * Borrado definitivo, con tres cierres:
 *  1) hay que teclear el código exacto del rol (no el nombre: el código es lo
 *     que las reglas usan, y es lo que se ve en la papelera);
 *  2) el motivo es obligatorio y de al menos 10 caracteres;
 *  3) solo el super llega hasta aquí, y el backend lo vuelve a comprobar.
 */
...
const okDisabled = (target) => !target
    || form.value.name_confirmation !== target.code
...
<template>
    ...
    >
        ...
        <p class="force-msg">
            {{ $t('global.force_delete_name_prompt', { name: target?.code }) }}
        </p>
        <Input
