...
import { document_typesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.document_types.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.document_types.restore',
    bulkRestoreRouteName: 'business_management.document_types.bulk_restore',
});

const columns = computed(() => document_typesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.document_types.total === 1 ? t('global.record') : t('global.records');
    return `${props.document_types.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('document_types.trash_title')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.document_types.index')"
            :title="$t('document_types.trash_title')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <Empty
                v-if="document_types.data.length === 0"
