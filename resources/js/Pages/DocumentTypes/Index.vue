...
import {
    document_typesFilterFields, document_typesEmptyFilters, hydrateDocumentTypesFilters,
    document_typesFiltersToQuery, document_typesFiltersSummary,
...
const props = defineProps({
    document_types:      { type: Object, required: true },
    filters:      { type: Object, default: () => ({}) },
    filterSchema: { type: Array,  default: () => [] },
});

// ─── Filtros (schema + (de)serialización en config/filters.js) ──────────────
const filterFields = computed(() => document_typesFilterFields(t));
...
const {
    ...
    hydrate:       hydrateDocumentTypesFilters,
    toQuery:       document_typesFiltersToQuery,
    summary:       document_typesFiltersSummary,
    empty:         document_typesEmptyFilters,
    only:          ['document_types', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.document_types),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    document_typesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.document_types.current_page,
    pageSize: props.document_types.per_page,
    total:    props.document_types.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.document_types.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'document_types_view_mode';
...
const currentSortLabel = computed(() =>
    sortOptions.value.find((o) => o.value === currentSort.value)?.label ?? t('document_types.code'),
);
const setSort = ({ key }) => {
...
useModuleUndoToast('business_management.document_types.undo_last_delete');
...
const tour = useModuleTour({ module: 'document_types', steps: () => moduleTourSteps(t, { moduleName: t('document_types.plural') }) });

useKeyboardShortcuts({
    'ctrl+n': () => can('document_types.create') && router.visit(route('business_management.document_types.create')),
...
<template>
    <Head :title="$t('document_types.plural')" />

    <div class="sap-index">
        <div class="mi-title" data-tour="module">
            <CatalogPageHeader :title="$t('document_types.plural')" :subtitle="$t('document_types.index_subtitle')">
        ...
        <div class="mi-console mi-console--v2">
            ...
            <span class="mi-colsel-host" aria-hidden="true">
                <ColumnSelector
                    ...
                    storage-key="document_types"
                />
            </span>
        </div>

        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="document_types" />
        </Drawer>
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input
                        v-model="quickSearch"
                        class="mi-bar__input"
                        :placeholder="$t('global.search_in', { item: $t('document_types.singular').toLowerCase() })"
                        autocomplete="off"
                        spellcheck="false"
                        type="text"
                    />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.document_types.trash'))">
                ...
                <Tooltip v-if="can('document_types.create')" :title="$t('document_types.new')" data-tour="new">
                    <Link :href="route('business_management.document_types.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('document_types.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="document_types.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('document_types.delete') || can('document_types.edit')) ? rowSelection : null"
        ...
        <CatalogBulkDeleteModal
            ...
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('document_types.record') : $t('document_types.records')"
