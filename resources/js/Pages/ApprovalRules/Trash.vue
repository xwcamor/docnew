...
import ApprovalRulesTrashBulkBar from '@/Components/ApprovalRules/ApprovalRulesTrashBulkBar.vue';
import ApprovalRulesForceDeleteModal from '@/Components/ApprovalRules/ApprovalRulesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { approvalRulesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.approval_rules.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.approval_rules.restore',
    bulkRestoreRouteName: 'business_management.approval_rules.bulk_restore',
});
...
const openForceDelete = (approvalRule) => {
    forceDeleteTarget.value = approvalRule;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.approval_rules.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => approvalRulesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.approval_rules.total === 1 ? t('global.record') : t('global.records');
    return `${props.approval_rules.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('approval_rules.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.approval_rules.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('approval_rules.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <Empty
                v-if="approval_rules.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <ApprovalRulesTrashBulkBar
