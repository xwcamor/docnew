...
import WorkTypesTrashBulkBar from '@/Components/WorkTypes/WorkTypesTrashBulkBar.vue';
import WorkTypesForceDeleteModal from '@/Components/WorkTypes/WorkTypesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { workTypesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.work_types.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.work_types.restore',
    bulkRestoreRouteName: 'business_management.work_types.bulk_restore',
});
...
const openForceDelete = (workType) => {
    forceDeleteTarget.value = workType;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.work_types.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => workTypesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.work_types.total === 1 ? t('global.record') : t('global.records');
    return `${props.work_types.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('work_types.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.work_types.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('work_types.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <Empty
                v-if="work_types.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <WorkTypesTrashBulkBar
