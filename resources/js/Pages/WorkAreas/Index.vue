...
import {
    workAreasFilterFields, workAreasEmptyFilters, hydrateWorkAreasFilters,
    workAreasFiltersToQuery, workAreasFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { workAreasTableColumns } from './config/columns';
...
const {
    ...
    hydrate:       hydrateWorkAreasFilters,
    toQuery:       workAreasFiltersToQuery,
    summary:       workAreasFiltersSummary,
    empty:         workAreasEmptyFilters,
    only:          ['work_areas', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.work_areas),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    workAreasTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.work_areas.current_page,
    pageSize: props.work_areas.per_page,
    total:    props.work_areas.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.work_areas.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'work_areas_view_mode';
...
const currentSortLabel = computed(() =>
    sortOptions.value.find((o) => o.value === currentSort.value)?.label ?? t('work_areas.name'),
);
const setSort = ({ key }) => {
...
useModuleUndoToast('business_management.work_areas.undo_last_delete');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.work_areas.bulk_set_active',
    bulkDeleteRoute:    'business_management.work_areas.bulk_delete',
    resourceLabel:      t('work_areas.records'),
    ...
    // El candado no es un caso raro aquí — el área que trajo la migración nace
    // bloqueada, así que sin esta condición la casilla se deja marcar y la
    // masiva devuelve «N saltados (bloqueados)».
...
const tour = useModuleTour({ module: 'work_areas', steps: () => moduleTourSteps(t, { moduleName: t('work_areas.plural') }) });

useKeyboardShortcuts({
    'ctrl+n': () => can('work_areas.create') && router.visit(route('business_management.work_areas.create')),
...
const goEdit   = (record) => router.visit(route('business_management.work_areas.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.work_areas.delete', record.slug));
</script>

<template>
    <Head :title="$t('work_areas.plural')" />

    <div class="sap-index">
        <div class="mi-title" data-tour="module">
            <CatalogPageHeader :title="$t('work_areas.plural')" :subtitle="$t('work_areas.index_subtitle')">
        ...
        <div class="mi-console mi-console--v2">
            ...
            <span class="mi-colsel-host" aria-hidden="true">
                <ColumnSelector
                    ...
                    storage-key="work_areas"
                />
            </span>
        </div>

        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="work_areas" />
        </Drawer>
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input
                        v-model="quickSearch"
                        class="mi-bar__input"
                        :placeholder="$t('global.search_in', { item: $t('work_areas.singular').toLowerCase() })"
                        autocomplete="off"
                        spellcheck="false"
                        type="text"
                    />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.work_areas.trash'))">
                ...
                <Tooltip v-if="can('work_areas.create')" :title="$t('work_areas.new')" data-tour="new">
                    <Link :href="route('business_management.work_areas.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('work_areas.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="work_areas.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('work_areas.delete') || can('work_areas.edit')) ? rowSelection : null"
            ...
            >
                <template #empty>
                    <CatalogEmptyState
                        ...
                        :can-create="can('work_areas.create')"
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <template v-if="column.key === 'name'">
                        <Link :href="route('business_management.work_areas.show', record.slug)" class="lead__name lead__link">
                            {{ record.name }}
                        </Link>
                    </template>
        ...
        <CatalogBulkDeleteModal
            ...
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('work_areas.record') : $t('work_areas.records')"
