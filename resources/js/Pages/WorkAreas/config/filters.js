 ...
 */
export const workAreasFilterFields = (t) => [
    { key: 'name',      label: t('work_areas.filter_name'), type: 'tags' },
    { key: 'is_active', label: t('work_areas.is_active'),   type: 'select', options: [
...
export const workAreasEmptyFilters = () => ({
    name: [],
    is_active: null,
    created_at: null,
});
...
export const hydrateWorkAreasFilters = (server) => ({
...
export const workAreasFiltersToQuery = (f) => ({
...
export const workAreasFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length) parts.push(`${t('work_areas.filter_name')}: ${f.name.join(', ')}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('work_areas.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
