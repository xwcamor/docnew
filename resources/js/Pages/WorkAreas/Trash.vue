...
import { workAreasTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.work_areas.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.work_areas.restore',
    bulkRestoreRouteName: 'business_management.work_areas.bulk_restore',
});

const columns = computed(() => workAreasTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.work_areas.total === 1 ? t('global.record') : t('global.records');
    return `${props.work_areas.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('work_areas.trash_title')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.work_areas.index')"
            :title="$t('work_areas.trash_title')"
            :subtitle="subtitle"
