...
import {
    workLocationsFilterFields, workLocationsEmptyFilters, hydrateWorkLocationsFilters,
    workLocationsFiltersToQuery, workLocationsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { workLocationsTableColumns } from './config/columns';
...
const props = defineProps({
    work_locations:      { type: Object, required: true },
    filters:      { type: Object, default: () => ({}) },
    filterSchema: { type: Array,  default: () => [] },
});

// ─── Filtros (schema + (de)serialización en config/filters.js) ──────────────
const filterFields = computed(() => workLocationsFilterFields(t));
...
const {
    ...
    hydrate:       hydrateWorkLocationsFilters,
    toQuery:       workLocationsFiltersToQuery,
    summary:       workLocationsFiltersSummary,
    empty:         workLocationsEmptyFilters,
    only:          ['work_locations', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.work_locations),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    workLocationsTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.work_locations.current_page,
    pageSize: props.work_locations.per_page,
    total:    props.work_locations.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.work_locations.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'work_locations_view_mode';
...
const currentSortLabel = computed(() =>
    sortOptions.value.find((o) => o.value === currentSort.value)?.label ?? t('work_locations.name'),
);
const setSort = ({ key }) => {
...
useModuleUndoToast('business_management.work_locations.undo_last_delete');
...
const tour = useModuleTour({ module: 'work_locations', steps: () => moduleTourSteps(t, { moduleName: t('work_locations.plural') }) });

useKeyboardShortcuts({
    'ctrl+n': () => can('work_locations.create') && router.visit(route('business_management.work_locations.create')),
...
const goEdit   = (record) => router.visit(route('business_management.work_locations.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.work_locations.delete', record.slug));
</script>

<template>
    <Head :title="$t('work_locations.plural')" />

    <div class="sap-index">
        <div class="mi-title" data-tour="module">
            <CatalogPageHeader :title="$t('work_locations.plural')" :subtitle="$t('work_locations.index_subtitle')">
        ...
        <div class="mi-console mi-console--v2">
            ...
            <span class="mi-colsel-host" aria-hidden="true">
                <ColumnSelector
                    ...
                    storage-key="work_locations"
                />
            </span>
        </div>

        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="work_locations" />
        </Drawer>
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input
                        v-model="quickSearch"
                        class="mi-bar__input"
                        :placeholder="$t('global.search_in', { item: $t('work_locations.singular').toLowerCase() })"
                        autocomplete="off"
                        spellcheck="false"
                        type="text"
                    />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.work_locations.trash'))">
                ...
                <Tooltip v-if="can('work_locations.create')" :title="$t('work_locations.new')" data-tour="new">
                    <Link :href="route('business_management.work_locations.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('work_locations.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            <ResponsiveTable
                :loading="isFetching"
                :dataSource="work_locations.data"
                :columns="columns"
                :pagination="tablePagination"
                :row-selection="(can('work_locations.delete') || can('work_locations.edit')) ? rowSelection : null"
            ...
            >
                <template #empty>
                    <CatalogEmptyState
                        ...
                        :can-create="can('work_locations.create')"
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <template v-if="column.key === 'name'">
                        <Link :href="route('business_management.work_locations.show', record.slug)" class="lead__name lead__link">
                            {{ record.name }}
                        </Link>
                    </template>
        ...
        <CatalogBulkDeleteModal
            ...
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('work_locations.record') : $t('work_locations.records')"
