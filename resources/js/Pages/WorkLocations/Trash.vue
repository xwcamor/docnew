...
import { workLocationsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.work_locations.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.work_locations.restore',
    bulkRestoreRouteName: 'business_management.work_locations.bulk_restore',
});

const columns = computed(() => workLocationsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.work_locations.total === 1 ? t('global.record') : t('global.records');
    return `${props.work_locations.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('work_locations.trash_title')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.work_locations.index')"
            :title="$t('work_locations.trash_title')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <Empty
                v-if="work_locations.data.length === 0"
