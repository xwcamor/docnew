...
/**
 * Filtros del catálogo de nacionalidades.
 *
 * `name` es el buscador general de la barra: el backend lo hace valer contra
 * «Nacionalidad», que es el único campo que el usuario reconoce de esta tabla.
 */
export const nationalitiesFilterFields = (t) => [
    { key: 'name',      label: t('nationalities.filter_name'), type: 'tags' },
    { key: 'is_active', label: t('nationalities.is_active'),   type: 'select', options: [
...
export const nationalitiesEmptyFilters = () => ({
    name: [],
    is_active: null,
    created_at: null,
});
...
export const hydrateNationalitiesFilters = (server) => ({
...
export const nationalitiesFiltersToQuery = (f) => ({
...
export const nationalitiesFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length) parts.push(`${t('nationalities.filter_name')}: ${f.name.join(', ')}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('nationalities.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
