...
import {
    nationalitiesFilterFields, nationalitiesEmptyFilters, hydrateNationalitiesFilters,
    nationalitiesFiltersToQuery, nationalitiesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { nationalitiesTableColumns } from './config/columns';
...
const props = defineProps({
    nationalities:      { type: Object, required: true },
    filters:      { type: Object, default: () => ({}) },
    filterSchema: { type: Array,  default: () => [] },
});

// ─── Filtros (schema + (de)serialización en config/filters.js) ──────────────
const filterFields = computed(() => nationalitiesFilterFields(t));
...
const {
    ...
    hydrate:       hydrateNationalitiesFilters,
    toQuery:       nationalitiesFiltersToQuery,
    summary:       nationalitiesFiltersSummary,
    empty:         nationalitiesEmptyFilters,
    only:          ['nationalities', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.nationalities),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    nationalitiesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.nationalities.current_page,
    pageSize: props.nationalities.per_page,
    total:    props.nationalities.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.nationalities.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const currentSortLabel = computed(() =>
    sortOptions.value.find((o) => o.value === currentSort.value)?.label ?? t('nationalities.code'),
);
const setSort = ({ key }) => {
...
useModuleUndoToast('business_management.nationalities.undo_last_delete');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.nationalities.bulk_set_active',
    bulkDeleteRoute:    'business_management.nationalities.bulk_delete',
    resourceLabel:      t('nationalities.records'),
    ...
    // El candado no es un caso raro aquí — las nacionalidades que trajo la
    // migración nacen bloqueadas, así que sin esta condición la casilla se deja
    // marcar, el usuario escribe el motivo del borrado y el servidor contesta
    // «N bloqueado(s) se omitieron» (docs/UI.md §6). El porqué ya se ve en la
    // fila: CatalogActionsCell pinta el candado con su explicación.
...
const tour = useModuleTour({ module: 'nationalities', steps: () => moduleTourSteps(t, { moduleName: t('nationalities.plural') }) });

useKeyboardShortcuts({
    'ctrl+n': () => can('nationalities.create') && router.visit(route('business_management.nationalities.create')),
...
const goEdit   = (record) => router.visit(route('business_management.nationalities.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.nationalities.delete', record.slug));
</script>

<template>
    <Head :title="$t('nationalities.plural')" />

    <div class="sap-index">
        <div class="mi-title" data-tour="module">
            <CatalogPageHeader :title="$t('nationalities.plural')">
        ...
        <div class="mi-console mi-console--v2">
            ...
            <span class="mi-colsel-host" aria-hidden="true">
                <ColumnSelector
                    ...
                    storage-key="nationalities"
                />
            </span>
        </div>

        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="nationalities" />
        </Drawer>
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input
                        v-model="quickSearch"
                        class="mi-bar__input"
                        :placeholder="$t('global.search_in', { item: $t('nationalities.singular').toLowerCase() })"
                        autocomplete="off"
                        spellcheck="false"
                        type="text"
                    />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.nationalities.trash'))">
                ...
                <Tooltip v-if="can('nationalities.create')" :title="$t('nationalities.new')" data-tour="new">
                    <Link :href="route('business_management.nationalities.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('nationalities.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
            ...
            >
                <template #empty>
                    <CatalogEmptyState
                        ...
                        :can-create="can('nationalities.create')"
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <template v-if="column.key === 'code'">
                        <Link :href="route('business_management.nationalities.show', record.slug)" class="lead__name lead__link">
                            {{ record.code }}
                        </Link>
                    </template>
                    ...
                    <CatalogActionsCell
                        ...
                        :compact="compact"
                        :is-super="isSuper"
                        :can-edit="can('nationalities.edit')"
        ...
        <CatalogBulkDeleteModal
            ...
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('nationalities.record') : $t('nationalities.records')"
