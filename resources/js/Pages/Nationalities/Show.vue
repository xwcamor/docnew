...
const isDeleted = computed(() => !!props.nationality.deleted_at);
    ...
    <div class="show-page sap-show">
        ...
        >
            ...
            <template #subtitle>
                <Space :size="6" wrap>
                    ...
                    <Tag v-else :color="nationality.is_active ? 'success' : 'default'" :bordered="false">
                        {{ nationality.is_active ? $t('global.active') : $t('global.inactive') }}
                    </Tag>
                </Space>
            </template>
            <template #actions>
        ...
        <Alert v-if="isDeleted" type="error" show-icon class="deleted-alert">
            ...
            <template #description>
                <div><strong>{{ $t('global.deleted_at') }}:</strong> {{ fmt(nationality.deleted_at) }}</div>
                <div v-if="nationality.deleter">
                    <strong>{{ $t('global.deleted_by') }}:</strong> {{ nationality.deleter.name }}
                </div>
                <div v-if="nationality.deleted_description">
                    <strong>{{ $t('global.delete_description') }}:</strong> {{ nationality.deleted_description }}
                </div>
            </template>
            <template v-if="isSuper" #action>
                <ViewDeletedButton module="nationalities" route-prefix="business_management" />
            </template>
        </Alert>
        ...
        <EntityShowTabs :show-history="canSeeAudit" :history-count="activity.length">
            <template #general>
                ...
                <Card :bodyStyle="{ padding: 18 }" class="info-card">
                    ...
                    <div class="spec-grid">
                        <div v-if="isSuper" class="spec-cell">
                            ...
                            <span class="spec-cell__value">{{ nationality.id }}</span>
                        </div>
                        <div v-if="isSuper" class="spec-cell">
                            <span class="spec-cell__label">Slug</span>
                            <span class="spec-cell__value"><code class="muted">{{ nationality.slug }}</code></span>
                        </div>
                        <div class="spec-cell">
                            <span class="spec-cell__label">{{ $t('nationalities.code') }}</span>
                            <span class="spec-cell__value">{{ nationality.code }}</span>
                        </div>
                        <div class="spec-cell">
                            <span class="spec-cell__label">{{ $t('nationalities.country') }}</span>
                            <span class="spec-cell__value">{{ nationality.country ?? '—' }}</span>
                        </div>
                        <div class="spec-cell">
                            <span class="spec-cell__label">{{ $t('nationalities.is_active') }}</span>
                            <span class="spec-cell__value">
                                <Tag :color="nationality.is_active ? 'success' : 'default'" :bordered="false">
                                    {{ nationality.is_active ? $t('global.active') : $t('global.inactive') }}
                                </Tag>
                            </span>
                        </div>
                ...
                <Card :bodyStyle="{ padding: 18 }" class="info-card">
                    <template #title>{{ $t('nationalities.usage_list_title') }}</template>
                    <Empty v-if="usages.length === 0" :description="$t('nationalities.usage_list_none')" />
