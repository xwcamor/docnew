...
import { nationalitiesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.nationalities.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.nationalities.restore',
    bulkRestoreRouteName: 'business_management.nationalities.bulk_restore',
});

const columns = computed(() => nationalitiesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.nationalities.total === 1 ? t('global.record') : t('global.records');
    return `${props.nationalities.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('nationalities.trash_title')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.nationalities.index')"
            :title="$t('nationalities.trash_title')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <Empty
                v-if="nationalities.data.length === 0"
