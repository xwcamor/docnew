...
const submit = () => {
    form.delete(route('business_management.nationalities.deleteSave', props.nationality.slug), {
        preserveScroll: true,
    });
};
...
const deactivate = () => {
    ...
    router.post(route('business_management.nationalities.deactivate', props.nationality.slug), {}, {
...
<template>
    <Head :title="$t('nationalities.delete_title')" />

    <DeletePage
        :back-href="route('business_management.nationalities.index')"
        :title="$t('nationalities.delete_title')"
        :subtitle="nationality.code"
    ...
    >
        <template #warning>
            ...
            <Alert v-if="blocked" type="error" show-icon class="del-mb">
                <template #message>{{ $t('nationalities.delete_title') }}</template>
                <template #description>
                    <p class="blocked-reason">{{ blockedReason }}</p>
                    <Button
                        v-if="nationality.is_active"
                        type="primary"
                        size="large"
                        :loading="deactivating"
                        @click="deactivate"
                    >
                        {{ $t('nationalities.deactivate_instead') }}
                    </Button>
                </template>
            </Alert>
        </template>
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('nationalities.code')">{{ nationality.code }}</DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('nationalities.country')">{{ nationality.country }}</DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('nationalities.usage_count')">{{ nationality.usage_count }}</DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('nationalities.is_active')">
                <Tag :color="nationality.is_active ? 'success' : 'error'" :bordered="false">
