...
const allColumns = computed(() =>
    companiesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.companies.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'companies_view_mode';
...
useModuleUndoToast('business_management.companies.undo_last_delete');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.companies.bulk_set_active',
    bulkDeleteRoute:    'business_management.companies.bulk_delete',
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.companies.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => companiesExportableColumns(t, { isSuper: isSuper.value }));
...
useKeyboardShortcuts({
    'ctrl+n': () => can('companies.create') && router.visit(route('business_management.companies.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.companies.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.companies.delete', record.slug));
</script>
    ...
    <div class="sap-index">
        ...
        <div class="mi-title" data-tour="module">
            <CompaniesPageHeader
                ...
                :subtitle="$t('companies.index_subtitle')"
            />
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="companies" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                ...
                <Tooltip v-if="can('companies.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('companies.create') && can('companies.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('companies.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.companies.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.companies.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
                ...
                <Tooltip v-if="can('companies.create')" :title="$t('companies.new')" data-tour="new">
                    <Link :href="route('business_management.companies.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('companies.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <ImportDialog
            ...
            :endpoint="route('business_management.companies.import')"
            :template-url="route('business_management.companies.import_template')"
