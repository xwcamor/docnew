...
const saveAll = () => {
    ...
    router.post(
        route('business_management.companies.edit_all.update'),
...
<template>
    ...
    <div class="edit-all sap-form">
        ...
        <div v-if="companies.total > companies.per_page" class="edit-pagination">
            <Pagination
        ...
        <EditAllFooter
            :discard-label="$t('companies.edit_all_discard')"
            :save-label="$t('companies.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('companies.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
