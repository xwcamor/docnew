...
const submit = () => {
    form.delete(route('business_management.form_templates.deleteSave', props.formTemplate.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('form_templates.singular')" />

    <DeletePage
        :back-href="route('business_management.form_templates.index')"
        :title="$t('global.delete') + ' ' + $t('form_templates.record')"
        :subtitle="formTemplate.name"
    ...
    >
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('form_templates.name')">{{ formTemplate.name }}</DeleteSummaryRow>
            <DeleteSummaryRow v-if="formTemplate.code" :label="$t('form_templates.code')">
                <code>{{ formTemplate.code }}</code>
            </DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('form_templates.is_active')">
                <Tag :color="formTemplate.is_active ? 'success' : 'error'" :bordered="false">
