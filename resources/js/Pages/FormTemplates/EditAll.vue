...
import { SaveOutlined, UndoOutlined, EditOutlined, FileOutlined } from '@ant-design/icons-vue';

import AppLayout from '@/Layouts/AppLayout.vue';
import SectionHeader from '@/Components/Common/SectionHeader.vue';
import EditAllFooter from '@/Components/Common/EditAllFooter.vue';
import FormTemplatesEditAllTable from '@/Components/FormTemplates/FormTemplatesEditAllTable.vue';
...
/**
 * Edit-All de FormTemplates. Mantenemos snapshot `original` + `draft` mutable
...
const source = computed(() => props.form_templates.data);
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.form_templates.edit_all.update'),
...
<template>
    <Head :title="$t('form_templates.edit_all_title')" />

    <div class="edit-all sap-form">
        <SectionHeader
            :back-href="route('business_management.form_templates.index')"
            :title="$t('global.edit_all') + ' — ' + $t('form_templates.plural')"
            :subtitle="$t('form_templates.edit_all_subtitle')"
        >
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('form_templates.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <FormTemplatesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
                :duplicate-rows="duplicateRows"
            />

        <div v-if="form_templates.total > form_templates.per_page" class="edit-pagination">
            <Pagination
                :current="form_templates.current_page"
                :pageSize="form_templates.per_page"
                :total="form_templates.total"
        ...
        <EditAllFooter
            ...
            :save-label="$t('form_templates.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('form_templates.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
