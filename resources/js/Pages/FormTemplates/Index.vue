...
import FormTemplatesBulkBar from '@/Components/FormTemplates/FormTemplatesBulkBar.vue';
import FormTemplatesBulkDeleteModal from '@/Components/FormTemplates/FormTemplatesBulkDeleteModal.vue';
import FormTemplatesFavoriteCell from '@/Components/FormTemplates/FormTemplatesFavoriteCell.vue';
import FormTemplatesPageHeader from '@/Components/FormTemplates/FormTemplatesPageHeader.vue';
import FormTemplatesActionsCell from '@/Components/FormTemplates/FormTemplatesActionsCell.vue';
import FormTemplatesEmptyState from '@/Components/FormTemplates/FormTemplatesEmptyState.vue';
...
// El toolbar inline de FormTemplates (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    formTemplatesFilterFields, formTemplatesEmptyFilters, hydrateFormTemplatesFilters,
    formTemplatesFiltersToQuery, formTemplatesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { formTemplatesTableColumns } from './config/columns';
import { formTemplatesExportableColumns, formTemplatesExportEndpoints } from './config/exports';
...
const props = defineProps({
    form_templates:      { type: Object, required: true },
...
const {
    ...
    hydrate:       hydrateFormTemplatesFilters,
    toQuery:       formTemplatesFiltersToQuery,
    summary:       formTemplatesFiltersSummary,
    empty:         formTemplatesEmptyFilters,
    only:          ['form_templates', 'filters'],
    t,
});
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.form_templates),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    formTemplatesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.form_templates.current_page,
    pageSize: props.form_templates.per_page,
    total:    props.form_templates.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.form_templates.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'form_templates_view_mode';
...
useModuleUndoToast('business_management.form_templates.undo_last_delete');

// ─── Favoritos polimorficos ────────────────────────────────────────────────
const { submitting: favoriteSubmitting, toggle: toggleFavorite } = useModuleFavorites('form_templates', 'form_templates');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.form_templates.bulk_set_active',
    bulkDeleteRoute:    'business_management.form_templates.bulk_delete',
    resourceLabel:      t('form_templates.records'),
...
const duplicate = (record) => {
    ...
    router.post(route('business_management.form_templates.duplicate', record.slug), {}, {
        preserveScroll: true,
...
const exportableColumns = computed(() => formTemplatesExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => formTemplatesExportEndpoints());
...
const tour = useModuleTour({ module: 'form_templates', steps: () => moduleTourSteps(t, { moduleName: t('form_templates.plural') }) });

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('form_templates.create') && router.visit(route('business_management.form_templates.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.form_templates.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.form_templates.delete', record.slug));
</script>

<template>
    <Head :title="$t('form_templates.plural')" />

    <div class="sap-index">
        <!-- Título + acciones (las acciones viven en el toolbar de la tabla). -->
        <div class="mi-title" data-tour="module">
            <FormTemplatesPageHeader
                :title="$t('form_templates.plural')"
                :subtitle="$t('form_templates.index_subtitle')"
            />
        </div>
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="form_templates" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('form_templates.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('form_templates.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('form_templates.create') && can('form_templates.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('form_templates.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.form_templates.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.form_templates.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
                ...
                <Tooltip v-if="can('form_templates.create')" :title="$t('form_templates.new')" data-tour="new">
                    <Link :href="route('business_management.form_templates.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('form_templates.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('form_templates.delete') || can('form_templates.edit')) ? rowSelection : null"
            ...
            >
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    ...
                    <template v-else-if="column.key === 'name'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.form_templates.show', record.slug)" class="lead__name lead__link">{{ record.name }}</Link>
        ...
        <FormTemplatesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('form_templates.record') : $t('form_templates.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('form_templates.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="form_templates.total ?? 0"
            :total-unfiltered="form_templates.total_unfiltered ?? form_templates.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.form_templates.import')"
            :template-url="route('business_management.form_templates.import_template')"
            :resource-label="$t('form_templates.records')"
        />
    </div>
</template>
