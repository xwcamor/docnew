...
import FormTemplatesTrashBulkBar from '@/Components/FormTemplates/FormTemplatesTrashBulkBar.vue';
import FormTemplatesForceDeleteModal from '@/Components/FormTemplates/FormTemplatesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { formTemplatesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.form_templates.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.form_templates.restore',
    bulkRestoreRouteName: 'business_management.form_templates.bulk_restore',
});
...
const openForceDelete = (formTemplate) => {
    forceDeleteTarget.value = formTemplate;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.form_templates.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => formTemplatesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.form_templates.total === 1 ? t('global.record') : t('global.records');
    return `${props.form_templates.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('form_templates.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.form_templates.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('form_templates.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('form_templates.restore_hint')"
            ...
            <Empty
                v-if="form_templates.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <FormTemplatesTrashBulkBar
