...
const submit = () => {
    form.delete(route('business_management.positions.deleteSave', props.position.slug), {
        preserveScroll: true,
    });
};
...
const deactivate = () => {
    ...
    router.post(route('business_management.positions.deactivate', props.position.slug), {}, {
...
<template>
    <Head :title="$t('positions.delete_title')" />
    ...
    >
        <template #warning>
            ...
            <Alert v-if="blocked" type="error" show-icon class="del-mb">
                <template #message>{{ $t('positions.delete_title') }}</template>
                <template #description>
                    ...
                    <Button
                        ...
                        @click="deactivate"
                    >
                        {{ $t('positions.deactivate_instead') }}
                    </Button>
                </template>
            </Alert>
        </template>
        ...
        <template #summary>
            <DeleteSummaryRow :label="$t('positions.code')">{{ position.code }}</DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('positions.country')">{{ position.country }}</DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('positions.usage_count')">{{ position.usage_count }}</DeleteSummaryRow>
            <DeleteSummaryRow :label="$t('positions.is_active')">
                <Tag :color="position.is_active ? 'success' : 'error'" :bordered="false">
