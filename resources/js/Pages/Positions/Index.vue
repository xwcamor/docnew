...
import {
    positionsFilterFields, positionsEmptyFilters, hydratePositionsFilters,
    positionsFiltersToQuery, positionsFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { positionsTableColumns } from './config/columns';
...
const filterFields = computed(() => positionsFilterFields(t));
...
const {
    ...
    hydrate:       hydratePositionsFilters,
    toQuery:       positionsFiltersToQuery,
    summary:       positionsFiltersSummary,
    empty:         positionsEmptyFilters,
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.positions),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    positionsTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

const tablePagination = computed(() => ({
    current:  props.positions.current_page,
    pageSize: props.positions.per_page,
    total:    props.positions.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.positions.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'positions_view_mode';
...
const currentSortLabel = computed(() =>
    sortOptions.value.find((o) => o.value === currentSort.value)?.label ?? t('positions.code'),
);
const setSort = ({ key }) => {
...
useModuleUndoToast('business_management.positions.undo_last_delete');
...
const tour = useModuleTour({ module: 'positions', steps: () => moduleTourSteps(t, { moduleName: t('positions.plural') }) });

useKeyboardShortcuts({
    'ctrl+n': () => can('positions.create') && router.visit(route('business_management.positions.create')),
...
const goEdit   = (record) => router.visit(route('business_management.positions.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.positions.delete', record.slug));
</script>

<template>
    <Head :title="$t('positions.plural')" />
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="positions" />
        </Drawer>
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input
                        v-model="quickSearch"
                        class="mi-bar__input"
                        :placeholder="$t('global.search_in', { item: $t('positions.singular').toLowerCase() })"
                        autocomplete="off"
                        spellcheck="false"
                        type="text"
                    />
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.positions.trash'))">
                ...
                <Tooltip v-if="can('positions.create')" :title="$t('positions.new')" data-tour="new">
                    <Link :href="route('business_management.positions.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('positions.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            <ResponsiveTable
                ...
                :row-selection="(can('positions.delete') || can('positions.edit')) ? rowSelection : null"
            ...
            >
                <template #empty>
                    <CatalogEmptyState
                        ...
                        :can-create="can('positions.create')"
                ...
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <template v-if="column.key === 'code'">
                        <Link :href="route('business_management.positions.show', record.slug)" class="lead__name lead__link">
                            {{ record.code }}
                        </Link>
                    </template>
        ...
        <CatalogBulkDeleteModal
            ...
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('positions.record') : $t('positions.records')"
