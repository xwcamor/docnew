...
/**
 * Filtros del catálogo de cargos.
 *
 * `name` es el buscador general de la barra: el backend lo hace valer contra
 * «Cargo», que es el único campo que el usuario reconoce de esta tabla.
 */
export const positionsFilterFields = (t) => [
    { key: 'name',      label: t('positions.filter_name'), type: 'tags' },
    { key: 'is_active', label: t('positions.is_active'),   type: 'select', options: [
...
export const positionsEmptyFilters = () => ({
    name: [],
    is_active: null,
    created_at: null,
});
...
export const hydratePositionsFilters = (server) => ({
...
export const positionsFiltersToQuery = (f) => ({
...
export const positionsFiltersSummary = (f, t) => {
    const parts = [];
    if (f.name?.length) parts.push(`${t('positions.filter_name')}: ${f.name.join(', ')}`);
    if (f.is_active !== null && f.is_active !== undefined) {
        parts.push(`${t('positions.is_active')}: ${f.is_active ? t('global.active') : t('global.inactive')}`);
    }
