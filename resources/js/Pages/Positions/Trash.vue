...
import { positionsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.positions.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.positions.restore',
    bulkRestoreRouteName: 'business_management.positions.bulk_restore',
});

const columns = computed(() => positionsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.positions.total === 1 ? t('global.record') : t('global.records');
    return `${props.positions.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('positions.trash_title')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.positions.index')"
            :title="$t('positions.trash_title')"
            :subtitle="subtitle"
