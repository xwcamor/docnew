...
import WorkPlansTrashBulkBar from '@/Components/WorkPlans/WorkPlansTrashBulkBar.vue';
import WorkPlansForceDeleteModal from '@/Components/WorkPlans/WorkPlansForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { workPlansTrashColumns } from './config/trashColumns';
...
const {
    ...
    restoreRouteName:     'business_management.work_plans.restore',
    bulkRestoreRouteName: 'business_management.work_plans.bulk_restore',
});
...
const openForceDelete = (workPlan) => {
    forceDeleteTarget.value = workPlan;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.work_plans.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => workPlansTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.work_plans.total === 1 ? t('global.record') : t('global.records');
    return `${props.work_plans.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('work_plans.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.work_plans.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('work_plans.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('work_plans.restore_hint')"
            ...
            <Empty
                v-if="work_plans.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <WorkPlansTrashBulkBar
