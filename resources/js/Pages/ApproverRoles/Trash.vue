...
import ApproverRolesTrashBulkBar from '@/Components/ApproverRoles/ApproverRolesTrashBulkBar.vue';
import ApproverRolesForceDeleteModal from '@/Components/ApproverRoles/ApproverRolesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { approverRolesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.approver_roles.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.approver_roles.restore',
    bulkRestoreRouteName: 'business_management.approver_roles.bulk_restore',
});
...
const openForceDelete = (approverRole) => {
    forceDeleteTarget.value = approverRole;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.approver_roles.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => approverRolesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.approver_roles.total === 1 ? t('global.record') : t('global.records');
    return `${props.approver_roles.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('approver_roles.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.approver_roles.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('approver_roles.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <Empty
                v-if="approver_roles.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <ApproverRolesTrashBulkBar
