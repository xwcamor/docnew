...
import ApproverRolesBulkBar from '@/Components/ApproverRoles/ApproverRolesBulkBar.vue';
import ApproverRolesBulkDeleteModal from '@/Components/ApproverRoles/ApproverRolesBulkDeleteModal.vue';
import ApproverRolesPageHeader from '@/Components/ApproverRoles/ApproverRolesPageHeader.vue';
import ApproverRolesActionsCell from '@/Components/ApproverRoles/ApproverRolesActionsCell.vue';
import ApproverRolesEmptyState from '@/Components/ApproverRoles/ApproverRolesEmptyState.vue';
...
// El toolbar inline de ApproverRoles (no usa ModuleToolbar) repite manualmente
// estos gates para no mostrar botones que no funcionan al user free/basic.
const { canUse: canUsePlanFeature } = usePlanFeatures();

import {
    approverRolesFilterFields, approverRolesEmptyFilters, hydrateApproverRolesFilters,
    approverRolesFiltersToQuery, approverRolesFiltersSummary,
    serializeSavedFilters, deserializeSavedFilters,
} from './config/filters';
import { approverRolesTableColumns } from './config/columns';
import { approverRolesExportableColumns, approverRolesExportEndpoints } from './config/exports';
...
const props = defineProps({
    approver_roles:      { type: Object, required: true },
...
const {
    ...
    hydrate:       hydrateApproverRolesFilters,
    toQuery:       approverRolesFiltersToQuery,
    summary:       approverRolesFiltersSummary,
    empty:         approverRolesEmptyFilters,
    only:          ['approver_roles', 'filters'],
    t,
});

// El buscador de la barra escribe en el primer término del filtro `name`, que
// el backend hace valer a la vez contra el código y los dos nombres.
...
const { counterLabel } = useModuleListMeta({
    pagination: computed(() => props.approver_roles),
    hasActiveFilters,
    t,
});
...
const allColumns = computed(() =>
    approverRolesTableColumns(t, { isSuper: isSuper.value, isMobile: isMobileScreen.value }),
);
const { visibleColumnKeys, columns } = useColumnPreferences(allColumns);

// ─── Paginacion + sort ──────────────────────────────────────────────────────
const tablePagination = computed(() => ({
    current:  props.approver_roles.current_page,
    pageSize: props.approver_roles.per_page,
    total:    props.approver_roles.total,
...
const applyInlineFilters = (cleaned) => {
    ...
    inlineTimer = setTimeout(() => {
        ...
        router.get(route('business_management.approver_roles.index'), data, {
            preserveScroll: true,
            preserveState: true,
...
const VIEW_KEY = 'approver_roles_view_mode';
...
useModuleUndoToast('business_management.approver_roles.undo_last_delete');
...
const {
    ...
    bulkSetActiveRoute: 'business_management.approver_roles.bulk_set_active',
    bulkDeleteRoute:    'business_management.approver_roles.bulk_delete',
    resourceLabel:      t('approver_roles.records'),
    // Un rol del sistema o en uso no se puede borrar: se saca de la selección
    // para que la barra masiva no prometa algo que el servidor va a rechazar.
    rowDisabled:      (r) => (!isSuper.value && r.tenant_id == null) || r.is_system || r.rules_count > 0,
});
...
const exportableColumns = computed(() => approverRolesExportableColumns(t, { isSuper: isSuper.value }));
const exportEndpoints   = computed(() => approverRolesExportEndpoints());
...
const tour = useModuleTour({ module: 'approver_roles', steps: () => moduleTourSteps(t, { moduleName: t('approver_roles.plural') }) });

// ─── Keyboard shortcuts ────────────────────────────────────────────────────
useKeyboardShortcuts({
    'ctrl+n': () => can('approver_roles.create') && router.visit(route('business_management.approver_roles.create')),
    'esc': () => {
...
// ─── Acciones ───────────────────────────────────────────────────────────────
const goEdit   = (record) => router.visit(route('business_management.approver_roles.edit',   record.slug));
const goDelete = (record) => router.visit(route('business_management.approver_roles.delete', record.slug));
</script>

<template>
    <Head :title="$t('approver_roles.plural')" />
    ...
    <div class="sap-index">
        ...
        <Drawer v-model:open="filtersOpen" :title="$t('global.filters')" placement="right" :width="380">
            <FilterBar :fields="filterFields" v-model="filters" storage-key="approver_roles" />
        </Drawer>
        ...
        <div class="mi-tabletoolbar">
            ...
            <div class="mi-tabletoolbar__right">
                <label class="mi-bar mi-bar--toolbar" :class="{ 'is-active': quickSearch }">
                    ...
                    <input v-model="quickSearch" class="mi-bar__input" :placeholder="$t('global.search_in', { item: $t('approver_roles.singular').toLowerCase() })" autocomplete="off" spellcheck="false" type="text" />
                ...
                <Tooltip v-if="can('approver_roles.export')" :title="$t('global.export_hint')" data-tour="export-import">
                ...
                <Dropdown :trigger="['click']" placement="bottomRight">
                    ...
                    <template #overlay>
                        <Menu>
                            <MenuItem v-if="can('approver_roles.create') && can('approver_roles.import') && canUsePlanFeature('imports')" key="import" @click="importOpen = true"><UploadOutlined /> {{ $t('global.import') }}</MenuItem>
                            <MenuItem v-if="can('approver_roles.edit') && canUsePlanFeature('edit_all')" key="editall" @click="router.visit(route('business_management.approver_roles.edit_all'))"><FormOutlined /> {{ $t('global.edit_all') }}</MenuItem>
                            <MenuItem v-if="isSuper" key="trash" @click="router.visit(route('business_management.approver_roles.trash'))"><InboxOutlined /> {{ $t('global.view_deleted') }}</MenuItem>
                ...
                <Tooltip v-if="can('approver_roles.create')" :title="$t('approver_roles.new')" data-tour="new">
                    <Link :href="route('business_management.approver_roles.create')">
                        <Button type="primary" class="mi-iconbtn mi-create-btn" :aria-label="$t('approver_roles.new')"><PlusOutlined /></Button>
                    </Link>
                </Tooltip>
            </div>
        </div>
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                ...
                :row-selection="(can('approver_roles.delete') || can('approver_roles.edit')) ? rowSelection : null"
            ...
            >
                <template #empty>
                    <ApproverRolesEmptyState
                        :has-filters="hasActiveFilters"
                        :can-create="can('approver_roles.create')"
                        @clear-filters="clearFilters"
                        @open-import="importOpen = true"
                    />
                </template>
                <template #bodyCell="{ column, record, text, isMobile, compact }">
                    <template v-if="column.key === 'name_es'">
                        <div class="lead">
                            <div class="lead__txt">
                                <Link :href="route('business_management.approver_roles.show', record.slug)" class="lead__name lead__link">{{ record.name_es }}</Link>
                                <span class="lead__sub">{{ record.name_en }}</span>
                            </div>
                        </div>
                    </template>

                    <!-- El código es la clave con la que las reglas nombran al rol. -->
                    <template v-else-if="column.key === 'code'">
                        <code class="mono">{{ record.code }}</code>
                        <Tag v-if="record.is_system" color="purple" :bordered="false" class="sys-tag">{{ $t('approver_roles.system') }}</Tag>
                    </template>

                    <!-- Cuántas reglas firman con este rol: es lo que decide si se puede borrar. -->
                    <template v-else-if="column.key === 'rules_count'">
                        <Tag v-if="record.rules_count > 0" color="blue" :bordered="false">{{ record.rules_count }}</Tag>
        ...
        <ApproverRolesBulkDeleteModal
            v-model:open="bulkOpen"
            v-model:reason="bulkReason"
            :count="selectedRowKeys.length"
            :submitting="bulkSubmitting"
            :error-msg="bulkError"
            :resource-label="selectedRowKeys.length === 1 ? $t('approver_roles.record') : $t('approver_roles.records')"
        ...
        <ExportDialog
            ...
            :default-title="$t('approver_roles.export_title')"
            :endpoints="exportEndpoints"
            :limits="exportLimits"
            :total-rows="approver_roles.total ?? 0"
            :total-unfiltered="approver_roles.total_unfiltered ?? approver_roles.total ?? 0"
        />

        <ImportDialog
            v-model:open="importOpen"
            :endpoint="route('business_management.approver_roles.import')"
            :template-url="route('business_management.approver_roles.import_template')"
            :resource-label="$t('approver_roles.records')"
        />
    </div>
</template>
