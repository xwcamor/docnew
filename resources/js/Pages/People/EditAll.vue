...
import PeopleEditAllTable from '@/Components/People/PeopleEditAllTable.vue';
...
/**
 * Edit-All de People. Mantenemos snapshot `original` + `draft` mutable
...
const saveAll = () => {
    ...
    router.post(
        route('business_management.people.edit_all.update'),
...
<template>
    <Head :title="$t('people.edit_all_title')" />
    ...
    <div class="edit-all sap-form">
        ...
        <div v-if="people.total > people.per_page" class="edit-pagination">
            <Pagination
        ...
        <EditAllFooter
            :discard-label="$t('people.edit_all_discard')"
            :save-label="$t('people.edit_all_save_all')"
            :discard-disabled="dirtyCount === 0"
            :save-disabled="dirtyCount === 0 || duplicateRows.size > 0"
            :submitting="submitting"
            :status-text="dirtyCount > 0 ? $tc('people.edit_all_changes', dirtyCount) : ''"
            @discard="discardAll"
            @save="saveAll"
        />
