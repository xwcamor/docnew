...
import PeopleTrashBulkBar from '@/Components/People/PeopleTrashBulkBar.vue';
import PeopleForceDeleteModal from '@/Components/People/PeopleForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { peopleTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.people.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.people.restore',
    bulkRestoreRouteName: 'business_management.people.bulk_restore',
});
...
const openForceDelete = (person) => {
    forceDeleteTarget.value = person;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.people.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => peopleTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.people.total === 1 ? t('global.record') : t('global.records');
    return `${props.people.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('people.plural')" />

    <div v-if="isSuper" class="sap-form trash-page">
        <SectionHeader
            :back-href="route('business_management.people.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('people.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('people.restore_hint')"
