...
class CompaniesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($company) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($companies) ? count($companies) : iterator_count($companies));
        ...
        $companyTable->addRow(800);
        $companyCell = $companyTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $companyCell->addText($title, [
