...
/**
 * Generates a styled .docx report of ApproverRoles using PhpWord directly
 ...
 */
class ApproverRolesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($approverRole) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($approver_roles) ? count($approver_roles) : iterator_count($approver_roles));

        $this->columnDefs = [
            'id'         => ['heading' => __('approver_roles.id'),         'value' => fn($c) => (string) $c->id],
            'code'       => ['heading' => __('approver_roles.code'),       'value' => fn($c) => (string) $c->code],
            'name_es'    => ['heading' => __('approver_roles.name_es'),    'value' => fn($c) => (string) $c->name_es],
            'name_en'    => ['heading' => __('approver_roles.name_en'),    'value' => fn($c) => (string) $c->name_en],
            'sort_order' => ['heading' => __('approver_roles.sort_order'), 'value' => fn($c) => (string) ($c->sort_order ?? '')],
            'is_active'  => ['heading' => __('approver_roles.is_active'),  'value' => fn($c) => $c->is_active ? __('global.active') : __('global.inactive')],
        ...
        $title         = $options['title']         ?? __('approver_roles.export_title');
        ...
        $approverRoleTable = $section->addTable([
            'cellMargin'   => 200,
            'borderSize'   => 0,
            'unit'         => \PhpOffice\PhpWord\SimpleType\TblWidth::TWIP,
        ]);
        $approverRoleTable->addRow(800);
        $approverRoleCell = $approverRoleTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $approverRoleCell->addText($title, [
            'name' => 'Calibri', 'size' => 22, 'bold' => true, 'color' => 'FFFFFF',
        ], ['spaceAfter' => 60]);
        $approverRoleCell->addText(
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('ApproverRolesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('ApproverRolesTable');
            ...
            // Data rows (zebra)
            foreach ($approver_roles as $i => $approverRole) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($approverRole);

                    if ($col === 'is_active') {
                        $color = $approverRole->is_active ? '1D7044' : 'C8281D';
