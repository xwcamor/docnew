 ...
 */
class ApproverRolesExport implements FromCollection, WithEvents, WithTitle
{
    protected $approver_roles;
    protected int $count;
    ...
    public function __construct($approver_roles, array $options = [], ?int $count = null)
    {
        ...
        $this->approver_roles = is_array($approver_roles) ? collect($approver_roles) : $approver_roles;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('approver_roles.id'),         'value' => fn($c, $i) => $c->id],
            'code'       => ['heading' => __('approver_roles.code'),       'value' => fn($c, $i) => $c->code],
            'name_es'    => ['heading' => __('approver_roles.name_es'),    'value' => fn($c, $i) => $c->name_es],
            'name_en'    => ['heading' => __('approver_roles.name_en'),    'value' => fn($c, $i) => $c->name_en],
            'sort_order' => ['heading' => __('approver_roles.sort_order'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('approver_roles.is_active'),  'value' => fn($c, $i) => $c->is_active ? __('global.active') : __('global.inactive')],
    ...
    {
        $title    = $this->options['title'] ?? __('approver_roles.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s · %s · %s',
        ...
        // Row 5+ — data
        $i = 0;
        foreach ($this->approver_roles as $approverRole) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($approverRole, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('approver_roles.export_title'), 0, 31);
    }
