 ...
 */
class FormTemplatesExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($form_templates, array $options = [], ?int $count = null)
    {
        ...
        $this->form_templates = is_array($form_templates) ? collect($form_templates) : $form_templates;
    ...
    {
        $title    = $this->options['title'] ?? __('form_templates.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s · %s · %s',
        ...
        // Row 5+ — data
        $i = 0;
        foreach ($this->form_templates as $formTemplate) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($formTemplate, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('form_templates.export_title'), 0, 31);
    }
