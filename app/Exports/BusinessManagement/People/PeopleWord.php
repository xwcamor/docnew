...
/**
 * Generates a styled .docx report of People using PhpWord directly
...
class PeopleWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($person) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($people) ? count($people) : iterator_count($people));

        $this->columnDefs = [
            'id'         => ['heading' => __('people.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('people.name'),      'value' => fn($c) => (string) $c->name],
            'lastname'      => ['heading' => __('people.lastname'),   'value' => fn($c) => (string) $c->lastname],
            'doc_type'      => ['heading' => __('people.doc_type'),   'value' => fn($c) => (string) $c->doc_type],
            'num_doc'       => ['heading' => __('people.num_doc'),    'value' => fn($c) => (string) $c->num_doc],
            'country'       => ['heading' => __('people.country'),    'value' => fn($c) => (string) ($c->country?->name ?? '—')],
            'roles'         => ['heading' => __('people.roles'),      'value' => fn($c) => $c->roles->where('is_active', true)->map(fn ($r) => __('people.role_' . $r->role))->join(', ')],
            'companies'     => ['heading' => __('people.companies'),  'value' => fn($c) => $c->companyLinks->map(fn ($l) => $l->company?->name)->filter()->join(', ')],
            'companies_count' => ['heading' => __('people.companies_count'), 'value' => fn($c) => (string) ($c->company_links_count ?? 0)],
            'has_biometric' => ['heading' => __('people.biometric'),  'value' => fn($c) => ($c->active_biometrics_count ?? 0) > 0 ? __('people.biometric_yes') : __('people.biometric_no')],
            'is_active'  => ['heading' => __('people.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('people.export_title');
        ...
        $personTable = $section->addTable([
            'cellMargin'   => 200,
            'borderSize'   => 0,
            'unit'         => \PhpOffice\PhpWord\SimpleType\TblWidth::TWIP,
        ]);
        $personTable->addRow(800);
        $personCell = $personTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $personCell->addText($title, [
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('PeopleTable', [
                'borderSize'      => 4,
            ...
            foreach ($people as $i => $person) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($person);

                    if ($col === 'is_active') {
                        $color = $person->is_active ? '1D7044' : 'C8281D';
