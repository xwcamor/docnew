 ...
 */
class PeopleExport implements FromCollection, WithEvents, WithTitle
{
    protected $people;
    protected int $count;
    ...
    public function __construct($people, array $options = [], ?int $count = null)
    {
        ...
        $this->people = is_array($people) ? collect($people) : $people;
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('people.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('people.name'),      'value' => fn($c, $i) => $c->name],
            'lastname'      => ['heading' => __('people.lastname'),   'value' => fn($c, $i) => (string) $c->lastname],
            'doc_type'      => ['heading' => __('people.doc_type'),   'value' => fn($c, $i) => (string) $c->doc_type],
            'num_doc'       => ['heading' => __('people.num_doc'),    'value' => fn($c, $i) => (string) $c->num_doc],
            'country'       => ['heading' => __('people.country'),    'value' => fn($c, $i) => (string) ($c->country?->name ?? '—')],
            'roles'         => ['heading' => __('people.roles'),      'value' => fn($c, $i) => $c->roles->where('is_active', true)->map(fn ($r) => __('people.role_' . $r->role))->join(', ')],
            'companies'     => ['heading' => __('people.companies'),  'value' => fn($c, $i) => $c->companyLinks->map(fn ($l) => $l->company?->name)->filter()->join(', ')],
            'companies_count' => ['heading' => __('people.companies_count'), 'value' => fn($c, $i) => (string) ($c->company_links_count ?? 0)],
            'has_biometric' => ['heading' => __('people.biometric'),  'value' => fn($c, $i) => ($c->active_biometrics_count ?? 0) > 0 ? __('people.biometric_yes') : __('people.biometric_no')],
            'is_active'  => ['heading' => __('people.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    public function collection()
    {
        ...
        foreach ($this->people as $person) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($person, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('people.export_title'), 0, 31);
    }
