 ...
 */
class ApprovalRulesExport implements FromCollection, WithEvents, WithTitle
{
    protected $approval_rules;
    protected int $count;
    ...
    public function __construct($approval_rules, array $options = [], ?int $count = null)
    {
        ...
        $this->columnDefs = [
            ...
            'is_active'      => ['heading' => __('approval_rules.is_active'),      'value' => fn($c, $i) => $c->is_active ? __('global.active') : __('global.inactive')],
    ...
    public function collection()
    {
        ...
        foreach ($this->approval_rules as $approvalRule) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($approvalRule, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('approval_rules.export_title'), 0, 31);
    }
