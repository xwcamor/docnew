...
/**
 * Generates a styled .docx report of ApprovalRules using PhpWord directly
 ...
 */
class ApprovalRulesWord
{
    /** Map column key â†’ ['heading' => string, 'value' => fn($approvalRule) => mixed] */
    ...
    public function generate(
        ...
        $count = $count !== null
            ? $count
            : (is_countable($approval_rules) ? count($approval_rules) : iterator_count($approval_rules));
        ...
        $this->columnDefs = [
            ...
            'is_active'      => ['heading' => __('approval_rules.is_active'),      'value' => fn($c) => $c->is_active ? __('global.active') : __('global.inactive')],
        ...
        $title         = $options['title']         ?? __('approval_rules.export_title');
        ...
        $approvalRuleTable = $section->addTable([
            'cellMargin'   => 200,
            'borderSize'   => 0,
            'unit'         => \PhpOffice\PhpWord\SimpleType\TblWidth::TWIP,
        ]);
        $approvalRuleTable->addRow(800);
        $approvalRuleCell = $approvalRuleTable->addCell(9000, [
            'bgColor' => self::COLOR_SHELL,
            'valign'  => 'center',
        ]);
        $approvalRuleCell->addText($title, [
            'name' => 'Calibri', 'size' => 22, 'bold' => true, 'color' => 'FFFFFF',
        ], ['spaceAfter' => 60]);
        $approvalRuleCell->addText(
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('ApprovalRulesTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('ApprovalRulesTable');
            ...
            // Data rows (zebra)
            foreach ($approval_rules as $i => $approvalRule) {
                $table->addRow(360);
                $isEven = $i % 2 === 1;
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($approvalRule);

                    if ($col === 'is_active') {
                        $color = $approvalRule->is_active ? '1D7044' : 'C8281D';
