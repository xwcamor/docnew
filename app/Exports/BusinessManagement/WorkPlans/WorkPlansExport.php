...
class WorkPlansExport implements FromCollection, WithEvents, WithTitle
{
    ...
    public function __construct($work_plans, array $options = [], ?int $count = null)
    {
        ...
        $this->columnDefs = [
            ...
            'name'       => ['heading' => __('work_plans.name'),      'value' => fn($c, $i) => $c->name],
            'code'       => ['heading' => __('work_plans.code'),      'value' => fn($c, $i) => $c->code],
            'num_os' => ['heading' => __('work_plans.num_os'), 'value' => fn($c, $i) => $c->sort_order ?? ''],
            'is_active'  => ['heading' => __('work_plans.is_active'), 'value' => fn($c, $i) => $c->state_text],
