...
namespace App\Imports\BusinessManagement\FormTemplates;
...
/**
 * Imports FormTemplates from .xlsx/.csv.
 *
 * Columns:
 ...
 *
 * form_templates es PER-TENANT: el import scope-a por tenant_id via el global scope de
 * BelongsToTenant (FormTemplate::create autorellena el tenant del actor).
 *
 ...
 *   - Si el plan del usuario tiene limite, contamos cuantos form_templates hay HOY +
...
class FormTemplatesImport implements ToCollection, WithHeadingRow
{
    ...
    public function __construct(
        ...
        $this->currentCount = FormTemplate::count();
    }
    ...
    public function collection(Collection $rows): void
    {
        ...
        try {
            ...
            $seenInFileByName = [];
            $seenInFileByCode = [];
            $newRecordsCount = 0; // contador de filas que crearian un nuevo formTemplate
     ...
     * Per-tenant: el global scope de BelongsToTenant limita al tenant del actor.
     */
    protected function findExistingByNameInsensitive(string $name): ?FormTemplate
    {
        $isPgsql = DB::getDriverName() === 'pgsql';
        $query   = FormTemplate::query();

        if ($isPgsql) {
            $query->whereRaw('unaccent(LOWER(form_templates.name)) = unaccent(LOWER(?))', [$name]);
        } else {
            $query->whereRaw('LOWER(form_templates.name) = LOWER(?)', [$name]);
        }
