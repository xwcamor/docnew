...
/**
 * PositionService — Qué hace cada persona en obra, y cuáles de esos cargos pueden firmar una aprobación.
 *
 * El filtrado y las reglas de borrado viven aqui y no en el modelo a proposito:
 * `Position` es un modelo de dominio que tambien usan los planes de trabajo y la
 ...
 * fila **en uso** no se borra. Borrarla dejaria colgado a todo lo que la nombra: personas con este cargo. Se avisa y se ofrece desactivarla, que es
...
class PositionService
{
    ...
    public static function filter(Builder $query, Request $request): Builder
    {
        ...
        // El buscador principal va contra «Cargo»: quien escribe en la
    ...
    public function usos(Position $m): int
    {
        return \App\Models\PersonCompanyLink::where('position_id', $m->id)->count();
    }
    ...
    public function contarUsos(EloquentCollection $filas): void
    {
        ...
        $uso1 = \App\Models\PersonCompanyLink::whereIn('position_id', $ids)
    ...
    public function usados(Position $m): array
    {
        return \App\Models\PersonCompanyLink::where('position_id', $m->id)
            ->with('person:id,slug,name,lastname')->limit(20)->get()
            ->filter(fn ($v) => $v->person !== null)
            ->map(fn ($v) => ['slug' => $v->person->slug, 'label' => $v->person->name . ' ' . $v->person->lastname, 'is_active' => (bool) $v->is_active])
            ->values()
            ->all();
    }
     ...
     */
    public function motivoParaNoBorrar(Position $m): ?string
    {
        $enUso = $this->usos($m);

        return $enUso > 0
            ? __('positions.in_use_cannot_delete', ['count' => $enUso])
            : null;
    }
    ...
    public function update(Position $m, array $data): Position
    {
        $m->update($data);
     ...
     */
    public function delete(Position $m, string $motivo): void
    {
    ...
    public function deactivate(Position $m): Position
    {
    ...
    public function restore(Position $m): Position
    {
    ...
    public function forceDelete(Position $m, string $motivo): void
    {
        DB::transaction(function () use ($m, $motivo) {
            $bloqueado = Position::onlyTrashed()->where('id', $m->id)->lockForUpdate()->first();
            if (! $bloqueado) {
                throw new \RuntimeException("Position {$m->id} ya no esta disponible para borrado definitivo");
            }
    ...
    {
        $filas = Position::whereIn('id', $ids)->get();
    ...
    public function bulkSetActive(array $ids, bool $activo): int
    {
        return DB::transaction(function () use ($ids, $activo) {
            $cambiados = 0;
            foreach (Position::whereIn('id', $ids)->get() as $fila) {
    ...
    public function bulkRestore(array $ids): int
    {
        return DB::transaction(function () use ($ids) {
            $filas = Position::onlyTrashed()->whereIn('id', $ids)->get();
