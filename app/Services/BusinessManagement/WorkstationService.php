...
/**
 * WorkstationService — Los puestos de cada sede. Un plan de trabajo se hace en uno de ellos.
 *
 * El filtrado y las reglas de borrado viven aqui y no en el modelo a proposito:
 * `Workstation` es un modelo de dominio que tambien usan los planes de trabajo y la
 ...
 * fila **en uso** no se borra. Borrarla dejaria colgado a todo lo que la nombra: planes que lo usan. Se avisa y se ofrece desactivarla, que es
...
class WorkstationService
{
    ...
    public function usos(Workstation $m): int
    {
        return \App\Models\WorkPlan::where('workstation_id', $m->id)->count();
    }
    ...
    public function contarUsos(EloquentCollection $filas): void
    {
        ...
        $uso1 = \App\Models\WorkPlan::whereIn('workstation_id', $ids)
            ->selectRaw('workstation_id, count(*) as total')
            ->groupBy('workstation_id')
            ->pluck('total', 'workstation_id');
     ...
     */
    public function motivoParaNoBorrar(Workstation $m): ?string
    {
        $enUso = $this->usos($m);

        return $enUso > 0
            ? __('workstations.in_use_cannot_delete', ['count' => $enUso])
            : null;
    }

    // ── Alta / baja / modificacion ───────────────────────────────────────────

    public function create(array $data): Workstation
    {
        $fila = new Workstation($data);
    ...
    private function slug(): string
    {
        ...
        } while (Workstation::withTrashed()->where('slug', $slug)->exists());

        return $slug;
    }

    public function update(Workstation $m, array $data): Workstation
    {
        $m->update($data);
     ...
     */
    public function delete(Workstation $m, string $motivo): void
    {
    ...
    public function deactivate(Workstation $m): Workstation
    {
    ...
    public function restore(Workstation $m): Workstation
    {
    ...
    public function forceDelete(Workstation $m, string $motivo): void
    {
        DB::transaction(function () use ($m, $motivo) {
            $bloqueado = Workstation::onlyTrashed()->where('id', $m->id)->lockForUpdate()->first();
            if (! $bloqueado) {
                throw new \RuntimeException("Workstation {$m->id} ya no esta disponible para borrado definitivo");
            }

            AuditLog::create([
                'user_id'        => auth()->id(),
                'auditable_type' => Workstation::class,
    ...
    {
        $filas = Workstation::whereIn('id', $ids)->get();
    ...
    public function bulkSetActive(array $ids, bool $activo): int
    {
        return DB::transaction(function () use ($ids, $activo) {
            $cambiados = 0;
            foreach (Workstation::whereIn('id', $ids)->get() as $fila) {
    ...
    public function bulkRestore(array $ids): int
    {
        return DB::transaction(function () use ($ids) {
            $filas = Workstation::onlyTrashed()->whereIn('id', $ids)->get();
