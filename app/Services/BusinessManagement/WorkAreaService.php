...
/**
 * WorkAreaService — La parte de la sede donde se trabaja: es lo que se anota en el plan.
 *
 * El filtrado y las reglas de borrado viven aqui y no en el modelo a proposito:
 * `WorkArea` es un modelo de dominio que tambien usan los planes de trabajo y la
 ...
 * fila **en uso** no se borra. Borrarla dejaria colgado a todo lo que la nombra: planes que la usan. Se avisa y se ofrece desactivarla, que es
...
class WorkAreaService
{
    ...
    public function usos(WorkArea $m): int
    {
        return \App\Models\WorkPlan::where('work_area_id', $m->id)->count();
    }
    ...
    public function contarUsos(EloquentCollection $filas): void
    {
        ...
        $uso1 = \App\Models\WorkPlan::whereIn('work_area_id', $ids)
            ->selectRaw('work_area_id, count(*) as total')
            ->groupBy('work_area_id')
            ->pluck('total', 'work_area_id');
    ...
    public function usados(WorkArea $m): array
    {
        return \App\Models\WorkPlan::where('work_area_id', $m->id)
     ...
     */
    public function motivoParaNoBorrar(WorkArea $m): ?string
    {
        $enUso = $this->usos($m);

        return $enUso > 0
            ? __('work_areas.in_use_cannot_delete', ['count' => $enUso])
            : null;
    }

    // ── Alta / baja / modificacion ───────────────────────────────────────────

    public function create(array $data): WorkArea
    {
        $fila = new WorkArea($data);
    ...
    private function slug(): string
    {
        ...
        } while (WorkArea::withTrashed()->where('slug', $slug)->exists());

        return $slug;
    }

    public function update(WorkArea $m, array $data): WorkArea
    {
        $m->update($data);
     ...
     */
    public function delete(WorkArea $m, string $motivo): void
    {
    ...
    public function deactivate(WorkArea $m): WorkArea
    {
    ...
    public function restore(WorkArea $m): WorkArea
    {
    ...
    public function forceDelete(WorkArea $m, string $motivo): void
    {
        DB::transaction(function () use ($m, $motivo) {
            $bloqueado = WorkArea::onlyTrashed()->where('id', $m->id)->lockForUpdate()->first();
            if (! $bloqueado) {
                throw new \RuntimeException("WorkArea {$m->id} ya no esta disponible para borrado definitivo");
            }

            AuditLog::create([
                'user_id'        => auth()->id(),
                'auditable_type' => WorkArea::class,
    ...
    {
        $filas = WorkArea::whereIn('id', $ids)->get();
    ...
    public function bulkSetActive(array $ids, bool $activo): int
    {
        return DB::transaction(function () use ($ids, $activo) {
            $cambiados = 0;
            foreach (WorkArea::whereIn('id', $ids)->get() as $fila) {
    ...
    public function bulkRestore(array $ids): int
    {
        return DB::transaction(function () use ($ids) {
            $filas = WorkArea::onlyTrashed()->whereIn('id', $ids)->get();
