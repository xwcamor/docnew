<?php

namespace App\Services\BusinessManagement;

use App\Jobs\BusinessManagement\FormTemplates\BulkFormTemplatesActionJob;
use App\Models\AuditLog;
use App\Models\FormTemplate;
use Illuminate\Support\Facades\DB;

/**
 * FormTemplateService â€” operaciones de negocio del modulo form_templates.
 *
 * Clon del patron de RegionService/RoleService: el controller queda thin
 * y delega aquí toda la mutacion de datos. Mantiene los audit logs cerca
 * de la operacion (Auditable trait dispara en created/updated/deleted/
 * restored; force_delete escribe el audit manual).
 *
 * NO maneja exports/imports/list: esa es orquestacion HTTP y vive en el
 * controller.
 */
class FormTemplateService
{
    public function create(array $data): FormTemplate
    {
        $formTemplate = new FormTemplate($data);
        $formTemplate->created_by = auth()->id();
        $formTemplate->save();
        return $formTemplate;
    }

    public function update(FormTemplate $formTemplate, array $data): FormTemplate
    {
        $formTemplate->update($data);
        return $formTemplate;
    }

    /**
     * Soft-delete con motivo. saveQuietly() evita un audit log `updated`
     * duplicado justo antes del `deleted`.
     */
    public function delete(FormTemplate $formTemplate, string $reason): void
    {
        $formTemplate->deleted_description = $reason;
        $formTemplate->deleted_by          = auth()->id();
        $formTemplate->is_active           = false;
        $formTemplate->saveQuietly();
        $formTemplate->delete();
    }

    public function restore(FormTemplate $formTemplate): FormTemplate
    {
        $formTemplate->deleted_description = null;
        $formTemplate->deleted_by          = null;
        $formTemplate->restore();
        return $formTemplate;
    }

    /**
     * Hard delete. Audit ANTES del delete (sobrevive al borrado) + transaccion
     * para atomicidad. lockForUpdate previene race con un restore concurrente.
     */
    public function forceDelete(FormTemplate $formTemplate, string $reason): void
    {
        DB::transaction(function () use ($formTemplate, $reason) {
            $locked = FormTemplate::onlyTrashed()->where('id', $formTemplate->id)->lockForUpdate()->first();
            if (!$locked) {
                throw new \RuntimeException("FormTemplate {$formTemplate->id} no longer available for force-delete");
            }

            AuditLog::create([
                'user_id'        => auth()->id(),
                'auditable_type' => FormTemplate::class,
                'auditable_id'   => $locked->id,
                'event'          => 'force_deleted',
                'old_values'     => [
                    'name' => $locked->name,
                    'code' => $locked->code,
                    'slug' => $locked->slug,
                ],
                'new_values'     => null,
                'url'            => request()?->fullUrl(),
                'ip_address'     => request()?->ip(),
                'user_agent'     => substr((string) request()?->userAgent(), 0, 500),
                'note'           => $reason,
                'module'         => 'form_templates',
                'created_at'     => now(),
            ]);

            $locked->forceDelete();
        });
    }

    /**
     * Clona el formTemplate. Sufijo "(copia)" con sanity guard de 100 intentos.
     * El `cod` no se copia (es unique por tenant â€” se deja en null para que
     * el usuario lo ajuste manualmente al editar el clon).
     */
    public function duplicate(FormTemplate $formTemplate): ?FormTemplate
    {
        $base    = $formTemplate->name . ' (' . __('global.duplicate_suffix') . ')';
        $isPgsql = DB::getDriverName() === 'pgsql';

        return DB::transaction(function () use ($formTemplate, $base, $isPgsql) {
            $candidate = $base;
            $i = 2;

            while (true) {
                $exists = FormTemplate::query()
                    ->when($isPgsql,
                        fn ($q) => $q->whereRaw('unaccent(LOWER(name)) = unaccent(LOWER(?))', [$candidate]),
                        fn ($q) => $q->whereRaw('LOWER(name) = LOWER(?)', [$candidate]),
                    )
                    ->lockForUpdate()
                    ->exists();

                if (!$exists) break;
                $candidate = $base . ' ' . $i;
                $i++;
                if ($i > 100) return null;
            }

            $clone = new FormTemplate($formTemplate->only(['is_active', 'version']));
            $clone->name       = $candidate;
            $clone->code       = null;
            $clone->created_by = auth()->id();
            $clone->save();

            return $clone;
        });
    }

    // â”€â”€â”€ Bulk ops â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    //
    // Auto-async: si count(ids) excede el umbral, dispatchamos el job y
    // devolvemos un payload "queued" para que el controller redirija con
    // mensaje de cola. Bajo el umbral, corre inline. El umbral vive en
    // BulkFormTemplatesActionJob::asyncThreshold() (Setting global -> config).

    public function shouldDispatchAsync(int $count): bool
    {
        return $count > BulkFormTemplatesActionJob::asyncThreshold();
    }

    /**
     * @return array{queued: bool, count: int, deleted?: int[]}
     */
    public function bulkDelete(array $ids, string $reason): array
    {
        $count = count($ids);

        if ($this->shouldDispatchAsync($count)) {
            BulkFormTemplatesActionJob::dispatch(
                (int) auth()->id(),
                'delete',
                $ids,
                ['reason' => $reason],
            );
            return ['queued' => true, 'count' => $count, 'deleted' => []];
        }

        return DB::transaction(function () use ($ids, $reason) {
            $form_templates  = FormTemplate::whereIn('id', $ids)->get();
            $deletedIds = [];
            foreach ($form_templates as $formTemplate) {
                $this->delete($formTemplate, $reason);
                $deletedIds[] = $formTemplate->id;
            }
            return ['queued' => false, 'count' => $form_templates->count(), 'deleted' => $deletedIds];
        });
    }

    /**
     * @return array{queued: bool, count: int, changed?: int}
     */
    public function bulkSetActive(array $ids, bool $isActive): array
    {
        $count = count($ids);

        if ($this->shouldDispatchAsync($count)) {
            BulkFormTemplatesActionJob::dispatch(
                (int) auth()->id(),
                'set_active',
                $ids,
                ['is_active' => $isActive],
            );
            return ['queued' => true, 'count' => $count];
        }

        return DB::transaction(function () use ($ids, $isActive, $count) {
            $form_templates = FormTemplate::whereIn('id', $ids)->get();
            $changed   = 0;
            foreach ($form_templates as $formTemplate) {
                if ((bool) $formTemplate->is_active === $isActive) continue;
                $formTemplate->update(['is_active' => $isActive]);
                $changed++;
            }
            return ['queued' => false, 'count' => $count, 'changed' => $changed];
        });
    }

    /**
     * @return array{queued: bool, count: int, restored?: int}
     */
    public function bulkRestore(array $ids): array
    {
        $count = count($ids);

        if ($this->shouldDispatchAsync($count)) {
            BulkFormTemplatesActionJob::dispatch(
                (int) auth()->id(),
                'restore',
                $ids,
                [],
            );
            return ['queued' => true, 'count' => $count];
        }

        return DB::transaction(function () use ($ids, $count) {
            $form_templates = FormTemplate::onlyTrashed()->whereIn('id', $ids)->get();
            foreach ($form_templates as $formTemplate) {
                $this->restore($formTemplate);
            }
            return ['queued' => false, 'count' => $count, 'restored' => $form_templates->count()];
        });
    }

    /**
     * Undo dentro del window de 60s. Defense in depth: solo restaura las filas
     * que matchean deleted_by = userId, no cualquier id del claim.
     *
     * @param int[] $claimIds
     * @return int[] ids efectivamente restaurados
     */
    public function undoLastDelete(array $claimIds, int $userId): array
    {
        $form_templates = FormTemplate::onlyTrashed()
            ->whereIn('id', $claimIds)
            ->where('deleted_by', $userId)
            ->get();

        $restored = [];
        foreach ($form_templates as $formTemplate) {
            $this->restore($formTemplate);
            $restored[] = $formTemplate->id;
        }
        return $restored;
    }

    /**
     * Batch update de name + is_active. Persistencia en transaccion para
     * atomicidad. Skip filas sin cambio real para evitar audit log noise.
     *
     * @return int touched count
     */
    public function editAllUpdate(array $changes): int
    {
        $touched = 0;

        DB::transaction(function () use ($changes, &$touched) {
            $ids   = array_column($changes, 'id');
            $byId  = FormTemplate::whereIn('id', $ids)->get()->keyBy('id');

            foreach ($changes as $change) {
                $formTemplate = $byId[$change['id']] ?? null;
                if (!$formTemplate) continue;

                $patch = array_filter(
                    array_intersect_key($change, array_flip(['name', 'is_active'])),
                    fn ($v) => $v !== null,
                );
                if (empty($patch)) continue;

                $hasChange = false;
                foreach ($patch as $k => $v) {
                    if ((string) $formTemplate->{$k} !== (string) $v) { $hasChange = true; break; }
                }
                if (!$hasChange) continue;

                $formTemplate->fill($patch)->save();
                $touched++;
            }
        });

        return $touched;
    }
}
