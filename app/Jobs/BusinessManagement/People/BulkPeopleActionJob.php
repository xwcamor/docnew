 ...
 * dispatch viven en PersonService.
 *
 ...
 */
class BulkPeopleActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:people:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/people.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('people.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(PersonService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkPeopleActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $people = match ($this->action) {
                'restore' => Person::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => Person::whereIn('id', $chunk)->get(),
            };

            foreach ($people as $person) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($person, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $person),
                        'restore'    => $service->restore($person),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkPeopleActionJob: error on person {$person->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkPeopleActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkPeopleActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkPeopleActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(PersonService $service, Person $person): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $person->is_active === $target) return;
        $service->update($person, ['is_active' => $target]);
    }
}
