 ...
 *
 * Clon de BaseRegionExportJob adaptado a FormTemplates (per-tenant, con relacion
 ...
 */
abstract class BaseFormTemplateExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('form_templates.export_job_memory_limit', '512M'));
     ...
     *
     * FormTemplates es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        $base = FormTemplate::query()->withoutGlobalScope('tenantOrGlobal');
        ...
        if ($this->tenantId !== null) {
            $base->where(function ($q) {
                $q->where('form_templates.tenant_id', $this->tenantId)
                  ->orWhereNull('form_templates.tenant_id');
            });
        }
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('form_templates.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('form_templates.name'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['code'])) {
            $out[] = ['label' => __('form_templates.code'), 'value' => (string) $f['code']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('form_templates.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('form_templates.export_filename'));
