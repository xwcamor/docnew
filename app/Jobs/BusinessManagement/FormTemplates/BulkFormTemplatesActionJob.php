 ...
 * dispatch viven en FormTemplateService.
 *
 ...
 */
class BulkFormTemplatesActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:form_templates:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/form_templates.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('form_templates.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(FormTemplateService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkFormTemplatesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $form_templates = match ($this->action) {
                'restore' => FormTemplate::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => FormTemplate::whereIn('id', $chunk)->get(),
            };

            foreach ($form_templates as $formTemplate) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($formTemplate, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $formTemplate),
                        'restore'    => $service->restore($formTemplate),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkFormTemplatesActionJob: error on formTemplate {$formTemplate->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkFormTemplatesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkFormTemplatesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkFormTemplatesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(FormTemplateService $service, FormTemplate $formTemplate): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $formTemplate->is_active === $target) return;
        $service->update($formTemplate, ['is_active' => $target]);
    }
}
