 ...
 *
 * Clon de BaseRegionExportJob adaptado a ApproverRoles (per-tenant, con relacion
 ...
 */
abstract class BaseApproverRoleExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('approver_roles.export_job_memory_limit', '512M'));
     ...
     *
     * ApproverRoles es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        if ($scope === 'selected' && !empty($this->options['selected_ids'])) {
            return $base->whereIn('approver_roles.id', $this->options['selected_ids']);
        }
        if ($scope === 'all') {
            return $base;
        }

        // El filtrado vive en el servicio: el modelo es de dominio y no carga
        // con los scopes de una pantalla. Le pasamos un Request armado a mano.
        return \App\Services\BusinessManagement\ApproverRoleService::filter(
            $base,
            new \Illuminate\Http\Request($this->options['filters'] ?? []),
        );
    }
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['name'])) {
            ...
            $out[] = ['label' => __('global.search'), 'value' => implode(', ', $names)];
        }
        if (!empty($f['code'])) {
            $out[] = ['label' => __('approver_roles.code'), 'value' => (string) $f['code']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('approver_roles.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('approver_roles.export_filename'));
