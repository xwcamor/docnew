 ...
 * dispatch viven en ApprovalRuleService.
 *
 ...
 */
class BulkApprovalRulesActionJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function uniqueId(): string
    {
        ...
        return "bulk:approval_rules:{$this->userId}:{$this->action}:{$idsHash}";
    }

    /**
     * Umbral configurable. Prioridad: Setting global -> config/approval_rules.php -> 200.
    ...
    public static function asyncThreshold(): int
    {
        return \App\Models\Setting::getInt(
            ...
            (int) config('approval_rules.bulk_async_threshold', 200),
        );
    }
    ...
    public function handle(ApprovalRuleService $service): void
    {
        ...
        if (!$user) {
            \Log::warning('BulkApprovalRulesActionJob: user not found, aborting', [
                'user_id' => $this->userId,
                'action'  => $this->action,
            ]);
        ...
        foreach (array_chunk($this->ids, 200) as $chunk) {
            $approval_rules = match ($this->action) {
                'restore' => ApprovalRule::onlyTrashed()->whereIn('id', $chunk)->get(),
                default   => ApprovalRule::whereIn('id', $chunk)->get(),
            };

            foreach ($approval_rules as $approvalRule) {
                try {
                    match ($this->action) {
                        'delete'     => $service->delete($approvalRule, $this->payload['reason'] ?? 'Bulk delete'),
                        'set_active' => $this->setActive($service, $approvalRule),
                        'restore'    => $service->restore($approvalRule),
                        default      => throw new \InvalidArgumentException("Unknown action: {$this->action}"),
                    };
                    $processed++;
                } catch (\Throwable $e) {
                    $errors++;
                    \Log::warning("BulkApprovalRulesActionJob: error on approvalRule {$approvalRule->id}", [
                        'error' => $e->getMessage(),
                    ]);
                }
            }
        ...
        \Log::info("BulkApprovalRulesActionJob completed", [
            'user_id'   => $this->userId,
            'action'    => $this->action,
            'processed' => $processed,
            'errors'    => $errors,
    ...
    {
        \Log::error("BulkApprovalRulesActionJob failed", [
            'user_id' => $this->userId,
            'action'  => $this->action,
            'total'   => count($this->ids),
    ...
    protected function notifyUser(string $status, ?string $error = null): void
    {
        try {
            ...
            \Log::warning('BulkApprovalRulesActionJob: notify failed', ['error' => $e->getMessage()]);
        }
    }

    protected function setActive(ApprovalRuleService $service, ApprovalRule $approvalRule): void
    {
        $target = (bool) ($this->payload['is_active'] ?? true);
        if ((bool) $approvalRule->is_active === $target) return;
        $service->update($approvalRule, ['is_active' => $target]);
    }
}
