 ...
 *
 * Clon de BaseRegionExportJob adaptado a ApprovalRules (per-tenant, con relacion
 ...
 */
abstract class BaseApprovalRuleExportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        ini_set('memory_limit', config('approval_rules.export_job_memory_limit', '512M'));
     ...
     *
     * ApprovalRules es per-tenant: aplicamos `withoutGlobalScopes()` y manualmente
    ...
    protected function buildQuery()
    {
        ...
        return \App\Services\BusinessManagement\ApprovalRuleService::filter(
            $base,
    ...
    protected function buildFiltersSummary(): array
    {
        ...
        if (!empty($f['approver_role'])) {
            $out[] = ['label' => __('approval_rules.approver_role'), 'value' => (string) $f['approver_role']];
        }
        if (isset($f['is_active']) && $f['is_active'] !== '' && $f['is_active'] !== null) {
            $bool = filter_var($f['is_active'], FILTER_VALIDATE_BOOLEAN);
            $out[] = ['label' => __('approval_rules.is_active'), 'value' => $bool ? __('global.active') : __('global.inactive')];
        }
    ...
    {
        $base = Str::slug($this->options['title'] ?? __('approval_rules.export_filename'));
